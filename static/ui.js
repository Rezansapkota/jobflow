...
async function approveJob(id) {
    ...
    const submit=$('#mode').value==='auto';
    await api('/api/review/approve',{id,review_token:job.review_token,submit});
    await refresh();
    toast(submit?'Approved and queued for automatic submission.':'Documents approved. Choose Automatic submission to apply.');
}
$('#mode').onchange=async()=>{
    render();
    if($('#mode').value!=='auto')return;
    try {
        const approved=state.jobs.filter(j=>selected.has(j.id)&&j.documents_approved&&j.status==='ready'&&!j.submission_requested&&!j.submission_in_progress);
        for(const job of approved) await approveJob(job.id);
    } catch(err) { toast(err.message); }
};
$('#profile-form').onsubmit=async e=>{e.preventDefault();try{const data=Object.fromEntries(new FormData(e.target));data.answers=JSON.parse(data.answers||'{}');await api('/api/profile',data);await refresh();$('#search-role').value=data.roles;$('#search-location').value=state.job_search_location||data.search_location;$('#profile-saved').textContent='Saved';toast('Profile saved. Prepare fresh resumes to use your changes.');}catch(err){toast(err.message);}};
