...
Enter certifications and licences one per line with the issuing organisation, awarded/expiry dates and conditions. They appear in the resume's **CERTIFICATIONS** section and are provided to Qwen for matching, summaries and cover letters. Recognised general certification text fields can be filled from the selected profile. Specific yes/no eligibility questions still need exact saved answers; the agent does not infer them or upload certificate evidence files. Qwen is instructed to consider expiry and not treat uncertain validity as confirmed.
